...
def test_net(
    ...
    img_resized, target_ratio, size_heatmap = imgproc.resize_aspect_ratio(
        image,
        # args.canvas_size,
        200,  # The canvas to paste the text onto?
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trained_model", default="weights/craft_mlt_25k.pth", type=str
    )
    parser.add_argument("--refiner_model", default=None, type=str)
    ...
    parser.add_argument("--show_time", default=False, type=str2bool)
    parser.add_argument("--poly", default=False, type=str2bool)
    ...
    parser.add_argument("--refine", default=False, type=str2bool)
    ...
    for k, image_path in enumerate(image_list):
        ...
        # mask_file = result_folder +
        from pathlib import Path

        mask_file = Path(result_folder) / "maps" / ("res_" + filename + "_mask.jpg")
        mask_file = str(mask_file)

        cv2.imwrite(mask_file, score_text)

        file_utils.saveResult(
            image_path, image[:, :, ::-1], polys, dirname=str(Path(mask_file).parent)
        )
